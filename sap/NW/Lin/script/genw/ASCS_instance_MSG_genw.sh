#!/bin/sh
#*******************************************************
#*      ASCS MSG           genw.sh   (Version : 4.1-1) *
#*******************************************************
...
if [ -n "${CHECK_MONNAME}" -a "${CHECK_MONNAME}" != "<DISKW>" ]; then
	...
	if [ $? -ne 0 ]; then
		/usr/sbin/clplogcmd -m "Skip monitoring ${INSTANCE} instance(msg) due to the monitor resource ${CHECK_MONNAME} is not normal." -l warn
		exit 0
	fi
fi
