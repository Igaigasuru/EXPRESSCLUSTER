#!/bin/sh
#*******************************************************
#*      ASCS ENQ           genw.sh   (Version : 4.1-1) *
#*******************************************************
...
if [ -n "${CHECK_MONNAME}" -a "${CHECK_MONNAME}" != "<DISKW>" ]; then
	...
	if [ $? -ne 0 ]; then
		/usr/sbin/clplogcmd -m "Skip monitoring ${INSTANCE} instance(enq) due to the monitor resource ${CHECK_MONNAME} is not normal." -l warn
		exit 0
	fi
fi
...
COLOR=`echo ${LINE} | cut -f3 -d, | sed 's/^[ \t]*//g'`
if [ "${COLOR}" != "GREEN" ]
then
	if [ "${COLOR}" != "YELLOW" ]
	then
